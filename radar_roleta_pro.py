...
# ==========================================
# INTEGRAÇÃO API — CORRIGIDA E PROTEGIDA
# ==========================================
def buscar_dados_roleta_url(roleta_nome):
    ...
    try:
        resp = requests.get(url_base, params=params, headers=headers, timeout=8)
...
for chave, padrao in [
    ...
    ("brk_rodadas_contagem", 0), ("brk_grupo_ativo", None)
]:
    if chave not in st.session_state:
        st.session_state[chave] = padrao

def processar_novo_numero(num_novo, roleta_nome, filtro_opcao):
...
if modo_operacao == "On-line (Captura Automática)":
    ...
    if agora - st.session_state.ultima_busca_api > 15:
        ...
        if novos_dados:
            ...
            if ultimo_atual is None:
                ...
                st.sidebar.success(f"🟢 Dados carregados: {len(novos_dados)} rodadas")
            elif ultimo_novo != ultimo_atual:
                st.success(f"🆕 NOVO NÚMERO → **{ultimo_novo}**")
                processar_novo_numero(ultimo_novo, roleta_selecionada, filtro_hibrido_opcao)
                st.session_state.historico = novos_dados
            st.sidebar.success(f"🟢 Conectado: **{roleta_selecionada}** — {len(st.session_state.historico)} rodadas")
        else:
            st.sidebar.warning("🟡 Sem resposta da API no momento")
    ...
    with st.sidebar.form(key="form_manual", clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Adicionar"):
            processar_novo_numero(num_input, roleta_selecionada, filtro_hibrido_opcao)
            st.rerun()
...
if st.session_state.sinal_ativo:
    ...
    etapa_nome = {0: "🎯 Entrada Direta", 1: "📈 Gale 1 (G1)", 2: "📊 Gale 2 (G2)"}.get(tentativa, f"🔄 Tentativa {tentativa}")

    st.warning(f"⚠️ **SINAL DISPARADO — {etapa_nome}** | Roleta: **{roleta_selecionada}** | Alvos: **{alvos_exibidos}**")

    if alvos_exibidos:
        cols_alvos = st.columns(len(alvos_exibidos))
        for idx, num in enumerate(alvos_exibidos):
            with cols_alvos[idx]:
                bg_cor = "#00AA00" if num == 0 else ("#FF2222" if num in NUMEROS_VERMELHOS else "#000000")
                st.markdown(f"<div style='background-color:{bg_cor}; color:#FFF; border-radius:8px; padding:15px; text-align:center; font-size:22px; font-weight:bold;'>{num}</div>", unsafe_allow_html=True)
...
if res_brk_painel["sinal_ativo"]:
    grupo_ativo = res_brk_painel["grupo_confirmado"]
elif st.session_state.brk_grupo_ativo:
    grupo_ativo = st.session_state.brk_grupo_ativo
...
with st.expander("📖 Manual Completo do Sistema — Regras e Funcionamento", expanded=False):
    ...
    **Limiares de disparo:**
    ...
    ---
    ### 🔐 Grupo BRK/Oculto
    - **Grupo A:** termina em 1, 2, 3 → [1,2,3,11,12,13,21,22,23,31,32,33]
    - **Grupo B:** termina em 4, 5, 6 → [4,5,6,14,15,16,24,25,26,34,35,36]
    - **Grupo C:** termina em 7, 8, 9, 0 → [7,8,9,10,17,18,19,20,27,28,29,30]
    - **Gatilho:** mesma dezena em 2 rodadas consecutivas → prioriza números ausentes do grupo
    - **Validade:** 4 rodadas para o grupo ativo
...
# ==========================================
# ⚙️ INFORMAÇÕES DE DEPURAÇÃO
# ==========================================
st.markdown("---")
with st.expander("🛠️ Informações Técnicas / Depuração", expanded=False):
    c1, c2, c3 = st.columns(3)
    c1.write(f"**Total rodadas:** {len(st.session_state.historico)}")
    c2.write(f"**Sinal ativo:** {'SIM' if st.session_state.sinal_ativo else 'NÃO'}")
    c3.write(f"**Última API:** {time.strftime('%H:%M:%S', time.localtime(st.session_state.ultima_busca_api))}")

    if st.session_state.sinal_ativo:
        st.write(f"Alvos: {st.session_state.alvos_sinal} | Etapa: {st.session_state.tentativa_atual}")

    if st.session_state.brk_grupo_ativo:
        st.write(f"Grupo BRK ativo: {st.session_state.brk_grupo_ativo} — Restam {st.session_state.brk_rodadas_contagem} rodadas")

    st.write(f"Roleta selecionada: **{roleta_selecionada}**")
    st.write(f"Filtro atual: **{filtro_hibrido_opcao}**")
