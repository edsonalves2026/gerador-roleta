...
def processar_tiro_certo_e_headshot(historico_completo, dados_brk_in, puxadores_dict, vizinhos_fisi_dict, quentes_100):
    """Constrói a lista de alvos EXCLUSIVAMENTE baseada nas posições 1, 2, 3 e 13 e registra os pesos para a Tabela."""
    ...
    # Expandir candidatos a partir APENAS das posições estratégicas
    ...
    # Avaliação de Peso e Registro (Para mostrar na Tabela Visual)
...
def aplicar_afunilamento_estrategico(alvos_brutos, padrao_nome, df_rank, res_tiro_certo):
    """Regras de Limite: Head-Shot (1-3), Tiro Certo (4-6), Geral (Máx 8)"""
...
if st.session_state.historico and len(st.session_state.historico) >= 30:
    ...
    # Motor que analisa as posições estratégicas e define pesos
    ...
    for idx in posicoes_idx:
        ...
        if num in res_motores["headshot"]: status_dezena = "🎯 HEAD-SHOT"
        elif num in res_motores["tiro_certo"]: status_dezena = "🔥 TIRO CERTO"
